                    ...
                    for l=1:jy % CV1 folds
                        ...
                        for h=1:nclass % Loop through binary comparisons
                            ...
                            for m = 1 : nP % parameter combinations
                                ...
                                if ~isempty(TEMPL)
                                    [~,~,~,~, TEMPL.TemplParams ] = nk_ReturnAtOptPos([], [], [], [], TEMPL.Param(hix), pnt);
                                end
                                ...
                                for u=1:ul
                                    ...
                                    if ~any(permfl)
                                        ...
                                        % Compute original weight map in input space
                                        [Tx, Psel, Rx, SRx, Cx, ~, PAx, WFx ] = nk_VisXWeight(inp, MD{h}{m}{k,l}{u}, Ymodel, modelTrL, varind, ParamX, Find, Vind, decompfl, memoryprob, [], Fadd);

                                        % Compute permuted weight maps
                                        fprintf(' | Permuting:\t');
                                        Tx_perm = cell(1,nM); Px_perm = zeros(1,nperms(1)); 
                                        if ~isempty(TEMPL), WFx_perm = cell(1,nM); end
                                        for n=1:nM
                                            Tx_perm{n} = zeros(size(Tx{n},1),nperms(n)); 
                                            if ~isempty(TEMPL), WFx_perm{n} = zeros(nperms(n), size(WFx{n},2)); end 
                                        end
                                        for perms = 1:nperms(1)
                                            if ~sigfl 
                                                % Train permuted model
                                            ...
                                            [TXperms, ~, ~, ~, ~, ~, ~, WFxperms] = nk_VisXWeight(inp, MDs, Ymodel_perm, L_perm, varind, ParamX, Find, Vind, decompfl, memoryprob, [], Fadd);
                                            for n=1:nM 
                                                Tx_perm{n}(:,perms) = TXperms{n}; 
                                                if ~isempty(TEMPL), WFx_perm{n}(perms,:) = WFxperms{n}; end 
                                            end
                                        end
                                        ...
                                        if ~isempty(TEMPL)
                                            indnanWF = isnan(WFx{n});
                                            if isempty(I1.VCV1WFMPERM{h})
                                                I1.VCV1WFMPERM{h} = zeros(ill, TEMPL.templmatrixsize(2));
                                            end
                                            I1.VCV1WFMPERM{h}(il(h),:) = sum(abs(WFx_perm{n}) >= abs(WFx{n}) ) / nperms(1);
                                            I1.VCV1WFMPERM{h}(il(h),indnanWF) = NaN;
                                        end
    ...
    for h=1:inp.nclass
        for n=1:nM
            ...
            if datatype ==1 || datatype==2
                ...
                [~, oCVRfile] = nk_GenerateNMFilePath( inp.rootdir, SAV.matname, 'CVR', [], inp.varstr, inp.id, operm, ofold, [],[], '.nii');
                [~, SignConstFDRfile] = nk_GenerateNMFilePath( inp.rootdir, SAV.matname, 'SignConstFDR', [], inp.varstr, inp.id, operm, ofold, [],[], '.nii');
                [~, oCVRthrfile] = nk_GenerateNMFilePath( inp.rootdir, SAV.matname, 'CVR-SignConstFDR-masked-05', [], inp.varstr, inp.id, operm, ofold, [],[], '.nii');
