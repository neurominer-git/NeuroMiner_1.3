...
%     end
    ...
    switch type
        ...
        case {'HLLE', 'HessianLLE'}
			...
			if isempty(varargin), [mappedA,mapping] = hlle(A, no_dims, 12, eig_impl);
        ...
        case {'DM', 'DiffusionMaps'}
			...
			if isempty(varargin), 
                [ mappedA, mapping ] = diffusion_maps(A, no_dims, 1, 1);
            elseif length(varargin) == 1, 
                [ mappedA, mapping ] = diffusion_maps(A, no_dims, varargin{1}, 1);
            else
                [ mappedA, mapping ] = diffusion_maps(A, no_dims, varargin{1}, varargin{2}); 
            end
            mapping.name = 'DM';
...
%             [mappedA, network, binary_data, mean_X, var_X] = train_autoencoder(A, net_structure);
		...
		case {'KLDA', 'KFDA', 'KernelLDA', 'KernelFDA', 'GDA'}
            ...
            if isempty(varargin), [mappedA, mapping] = gda(A(:,2:end), uint8(A(:,1)), no_dims);
            else [mappedA, mapping] = gda(A(:,2:end), uint8(A(:,1)), no_dims, varargin{:}); end
