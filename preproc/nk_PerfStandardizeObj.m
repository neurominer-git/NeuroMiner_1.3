...
if eIN || ~IN.trained
    ...
    switch IN.method
        ...
        case {'standardization using median', 'standardization using mean'}
            ...
            funmean = @nm_nanmean; funstd = @nm_nanstd; 
            if strcmp(IN.method, 'standardization using median') 
                if isfield(IN,'IQRFUN') && IN.IQRFUN == 1
                    funstd = @iqr; 
                end
                funmean = @nm_nanmedian;     
            end
            ...
            for i=1:nG    
                ...
                IN.meanY(i,:) = funmean(Y(IN.sIND(:,i),:)); % compute mean
                nanvec = isnan(IN.meanY(i,:));
                if any(nanvec), fnanvec = find(nanvec); IN.meanY(i,fnanvec) = funmean(Y(IN.sIND(:,i),fnanvec)); end
                IN.stdY(i,:) = funstd(Y(IN.sIND(:,i),:)); % and IQR/STD
                std_zeros = find(IN.stdY(i,:)<=realmin);
                if ~isempty(std_zeros)
                    fprintf('\tWarning: %g non-discriminative feature(s) exist in the data! ', numel(std_zeros));
