...
global MODEFL MULTI CV RAND VERBOSE PREPROC STACKING SVM SYNTH
...
if isfield(inp,'oocvind') && ~isempty(inp.oocvind)
    ...
    if smiss>0
       ...
       error('Aborting stacker prepocessing module due to missing input features')
        ...
        OOCVDD = struct('RootPath',[],'FileNames',[], 'GD', [], 'CNT', [], 'CV1CNT', []);
    end
else
    oocvflag = false;
end
    ...
    for jm=1:nM
        if nM>1, bgstr = 'bagged '; else, bgstr=''; end
         ...
         if oocvflag
            ...
            load(OOCVpath,'binOOCVDh','cntOOCVDh','cntCV1OOCVdh'); 
            OOCVDD(cnt).GD          = binOOCVDh; 
            OOCVDD(cnt).CNT         = cntOOCVDh; 
            OOCVDD(cnt).CV1CNT      = cntCV1OOCVdh; 
         end
         cnt=cnt+1;
    end
end
...
for k=sta_iy:stp_iy % Inner permutation loop
    ...
    for l=sta_jy:stp_jy % Inner CV fold loop
        ...
        for u=1:ukbin % Binary comparison loop depending on PREPROC.BINMOD 
            ...
            switch MODEFL
                case 'classification'
                    if BINMOD == 1 && ~MULTI.flag 
                        [indtr, TrL] = create_binary_labels(labels(TrInd(CV.cvin{i,j}.TrainInd{k,l})), CV.class{i,j}{u});
                        TrI = TrInd(CV.class{i,j}{u}.TrainInd{k,l}); TrI(~indtr) = [];
                        [indcv, CVL] = create_binary_labels(labels(TrInd(CV.cvin{i,j}.TestInd{k,l})), CV.class{i,j}{u});
                        CVI = TrInd(CV.class{i,j}{u}.TestInd{k,l}); CVI(~indcv) = [];
                    else
                ...
                for jm = 1:nM
                    ...
                    for n=1:nP
                            ...
                            case 2
                                ...
                                for lll = 1:jy
                                    switch MODEFL
                                        case 'classification'
                                            if BINMOD == 1 && ~MULTI.flag 
                                                indcv = create_binary_labels(labels(TrInd(CV.cvin{i,j}.TestInd{k,lll})), CV.class{i,j}{u});
                                                CVIll = TrInd(CV.class{i,j}{u}.TestInd{k,lll}); CVIll(~indcv) = [];
                                            else
                                               CVIll = TrInd(CV.cvin{i,j}.TestInd{k,lll});
                                            end
                                        case 'regression'
                                            CVIll = TrInd(CV.cvin{i,j}.TestInd{k,lll});
                                    end
                                   oot_ind = [oot_ind; CVIll];
                                ...
                                [~,oot_order] = ismember(TrI,oot_ind);
                                i0 = oot_order == 0;
                                if any(i0), oot_order(i0) = TrI(i0); end
                        ...
                        if size(GDD(cnt).GD.DV{Pspos(n)}{k,l,u},2)>1
                            ...
                            if oocvflag 
                                % Bug fix 15/05/2024: The indexing of the
                                % CV1 predictions in the OOCV data was
                                % wrong. nk_OOCV concatenates decision
                                % scores at the CV1 level in the order of
                                % hyperparamater position => CV1
                                % permutation => CV1 fold. However here, in
                                % nk_PerfPreprocessMeta.m the
                                % hyperparameter positon is the inner-most
                                % loop, and therefore we need to compute
                                % a "jump-index" using "ll+(n-1)*nCV1p"
                                % where ll is the current CV1 partition, n
                                % the current hyperparameter combination
                                % and nCV1p the number of CV1 partitions 
                                Is = OOCVDD(cnt).CV1CNT{u}(ll+(n-1)*nCV1p,1);
                                Ie = OOCVDD(cnt).CV1CNT{u}(ll+(n-1)*nCV1p,2);
            ...
            if oocvflag , [InputParam.Ts{4}, ~, SrcParam.iOCV] = nk_ManageNanCases(ocv); end
            ...
            if SYNTH.flag == 1
                ...
                if SYNTH.write2disk
                    ...
                    synthfile = nk_GenerateNMFilePath(synthdir, SAV.matname, 'SynthDataMeta','', inp.varstr, inp.id, i, j, k, l );
                ...
                if file_fnd
                    ...
                    switch SYNTH.method
                        case 1
                            [ vTrSyn, LabelSyn, CovarsSyn ] = nk_SynthDistkNN(InputParam.Tr, TrL, Covs, SYNTH);
                            Synth_activated = true;
                        case 2
                            [ vTrSyn, LabelSyn, CovarsSyn, Synth_activated ] = nk_PerfADASYN( InputParam.Tr, TrL, Covs, SYNTH, true);
                        case 3
                            [ vTrSyn, LabelSyn, CovarsSyn ] = nk_SynthPCAGauss(InputParam.Tr, TrL, Covs, SYNTH);
                            Synth_activated = true;
                    end 
                    if SYNTH.write2disk
            ...
            if isfield(paramfl,'PREPROC') && isfield(paramfl,'P') && isfield(paramfl,'PXfull') && ~isempty(paramfl.PXopt{u})
            ...
            switch BINMOD
                ...
                case 0 
                    if ukbin > 1
                        ...
                        if oocvflag, tYocv.Ts{k,l}{u} = nk_ManageNanCases(ocv, [], SrcParam.iOCV); end
                    else
                        ...
                        if oocvflag, tYocv.Ts{k,l} = nk_ManageNanCases(ocv, [], SrcParam.iOCV); end
                    end
                ...
                case 1
                    ...
                    if ~strcmp(MODEFL,'regression') && length(CV.class{i,j}{u}.groups) == 2
                        [indtr, binlabels_tr] = create_binary_labels(TrL, CV.class{i,j}{u});
                        [indcv, binlabels_cv] = create_binary_labels(CVL, CV.class{i,j}{u});
                    else
                    ...
                    tY.VI{k,l}{u} = cell(size(trd,1),1);
                    % Transponse of chn removed on 03/03/2024
...
bin_ind = ( labels == class.groups(1) | labels == class.groups(2) | isnan(labels) );
