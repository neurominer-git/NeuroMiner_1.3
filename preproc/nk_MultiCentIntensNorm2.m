function [sY, IN, params] = nk_MultiCentIntensNorm2(Y, V, G, B, Brange, IN, flags)
...
if width(G) == 1% Check if G is a single vector with number indiciating centers / groups
   U = unique(G); lx = length(G); nclass = length(U); 
   tO = false(lx,nclass); tO(sub2ind([lx nclass], 1:lx, G')) = true;
   G = tO;
elseif exist('flags', 'var') && flags.corrmethod == 1 && ~islogical(G)
    G = logical(G);
end
...
if flags.train
    ...
    switch flags.corrmethod 
        ...
        case 3
            ...
            IN.sTrInd = G(I,:);
            IN.dTrInd = G(I,:);
            [~, IN] = nk_PerfRemMeanDiffObj(Y(I,:), IN);
            meanY = IN.meanY; 
            meanG = IN.meanG;
...
switch flags.corrmethod 
    ...
    case 3
        ...
        %if isfield(IN,'MS'), IN = rmfield(IN,'MS'); end
        sY = nk_PerfRemMeanDiffObj(Y, IN);
end
if flags.display
    figure; 
