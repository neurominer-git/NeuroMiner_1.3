function [ IN, optD, optCrit ] = nk_OptPredSeq(D, L, PredGroups, IN, C, nCutOff, Lims, OPTCRIT, Ddesc)
% =====================================================================================
% FORMAT function R = nk_OptPredSeq(D, L, PredGroups, IN, C nCutOff, Lims, OPTCRIT, Ddesc)
% =====================================================================================
...
%                threshold bounds
% OPTCRIT [opt] :   The optimization criterion such as BAC
...
%                fields:
%   IN.OPTCRIT :    Chosen optimization criterion (e.g. BAC)
%   IN.(OPTCRIT) :  Optimization criterion computed for each sequences to be 
...
if ~exist('IN','var') || isempty(IN)
    ...
    for j=1:nC
        ...
        switch SVM.SEQOPT.AnchorType
            case 1 % Flexible anchoring based on the decision boundaries of the models. This is for classification models only
                ...
                for z=1:nD % Loop through models
                    ...
                    pLthr = min(jD(:,z)); pUthr=max(jD(:,z));
                    % find subjects below the boundary
                    Lthr = jD( jD(:,z) < 0, z ); 
                    if ~isempty(Lthr)
                        % Compute lower percentile for case propagation
                        pLthr = percentile(Lthr, 100 - Lims(1)); 
                    else
                        Anchor(z) = 50;
                    end
                    LL(z) = sum(jD(:,z) <= pLthr)*100/m; 

                    % find subjects above the boundary
                    Uthr = jD( jD(:,z) > 0, z ); 
                    if ~isempty(Uthr)
                        % Compute upper percentile for case propagation
                        pUthr = percentile(Uthr, Lims(2)); 
                    else
                        Anchor(z) = 50;
                    end
                    UL(z) = sum(jD(:,z) <= pUthr)*100/m; 
                end
        ...
        tIN = struct('Sequence', Cj, ...
                ...
                'optlthr', 0, ...
                'optuthr', 0);
        tIN.(char(OPTCRIT))(1) = OPT;
        tIN.vecneg = vecneg;
        tIN.vecpos = vecpos;
        
        % Run sequential optimization algoríthm
        for i=1:nD-1, tIN = OptPredSeq(tIN, i, L, OPTCRIT); end
        ...
        if VERBOSE, fprintf('\nOptimized sequence performance: %1.2f', tIN.(char(OPTCRIT))(end)); end
    ...
    if lbmode 
        ...
        IN.SeqPerfGain_test(1)  = IN.OPTCRIT(L,D(:,1));
    end
    for j=1:numel(IN.AnalSeq)-1
        ...
        if lbmode 
            ...
            IN.SeqPerfGain_test(j+1) = IN.OPTCRIT(L,optD);
        end
        IN.optDh(:,j+1) = optD;
    end
...
% _________________________________________________________________________
function R = OptPredSeq(R, I, L, OPTCRIT)
global VERBOSE SVM
        ...
        case 1
            ...
            switch SVM.SEQOPT.PerfMode
                case 1 % based on prediction performance criterion 
                    prevOPT = OPTCRIT(tL,jD(fII));   % get current performance
                    ijOPT = OPTCRIT(tL,nD(fII));     % get performance if predictions of cases to be propagated (based on fII) are replaced with predictions of next model in the chain
