...
:::{note}
### Small‑batch safeguards (NM 1.4)
To stabilize **stratified CV** with rare strata:
- **Remove n=1 batches** from CV1 training partitions (guard against singleton strata).
- **Move batches with N ≤ 5** to **test folds** to avoid leakage and optimistic training.
- **Logging**: NM writes a **CV cycle generation log** listing fold membership, removed singletons, and relocated batches for reproducibility.

Enable under: *Cross‑validation settings* → **Stratification options** → **Small‑batch handling**.
:::
