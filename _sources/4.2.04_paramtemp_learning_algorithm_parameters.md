...
## New in NM 1.4 — Optimization strategies

**Simulated Annealing & Bayesian Optimization**  
In addition to grid/random search, NM 1.4 supports:
- **Simulated Annealing (SA):** global, temperature‑controlled search that can escape local optima. Recommended for large, rugged hyperparameter spaces.
- **Bayesian Optimization (BO):** surrogate‑model search (Gaussian Process or Tree‑based) that trades off exploration vs. exploitation via an acquisition function.
