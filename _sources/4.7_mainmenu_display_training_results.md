...
Many powerful machine learning models, such as Support Vector Machines (SVMs), produce decision scores rather than true probabilities. While these scores are excellent for ranking subjects (e.g., who is most likely to be a patient), they are not well-calibrated. A score of "0.8" does not mean there is an 80% chance of being in the positive class.

Probability calibration is a post-processing step that corrects this. It builds a mapping model that converts these arbitrary decision scores into meaningful probabilities. A well-calibrated model is one where a prediction of "80% probability" means that, out of all cases given that score, 80% of them actually belong to the positive class. This is crucial for clinical decision-making, risk assessment, and interpreting model confidence.

NeuroMiner applies this calibration by training a calibration model on the CV1 training scores and then applying it to the CV1/CV2 test scores. It offers three distinct methods for this transformation.
...
3. Bayesian Binning into Quantiles (BBQ)
...
Net Benefit is the primary metric in DCA. It combines the benefits of correctly identifying positive cases (true positives) with the harms of incorrectly identifying negative cases as positive (false positives).
