...
## Calibration & Decision‑Curve Analysis (NM 1.4)

- **Probability calibration** for non‑probabilistic models (e.g., SVM): Platt scaling or isotonic regression fitted in CV1 and applied to CV1/CV2 test predictions.
- **Calibration curves**: reliability plots + Expected Calibration Error (ECE).
- **Decision‑Curve Analysis (DCA)**: net benefit vs. threshold to evaluate clinical utility.


### Interpreting calibration & DCA

- **Calibration curve**: closer to the diagonal is better; report ECE and note over/under‑confidence regions.  
- **Platt vs. Isotonic**: Platt (parametric) is robust on small N; Isotonic (non‑parametric) is flexible but may overfit on very small N.  
- **DCA**: inspect net‑benefit vs. threshold; compare to “treat all / treat none”; discuss clinically relevant threshold bands.
