...
## Choosing a back‑projection mode (NEW, NM 1.4)

When visualizing latent‑space models (PCA/ICA/NMF):

- **Aggregated map**: a single summary map \(w_{\mathrm{agg}}=\sum_c w_c v_c\).  
- **Component‑wise maps**: factor‑resolved maps \(w^{(\mathrm{backproj})}_c=w_c v_c\) for components discovered in model space (FDR‑controlled).

For differences in **statistics, permutation testing, and multi‑modal shares**, see the dedicated page: [Visualization Pathways in NM 1.4](4.6.11_visualization_pathways_backprojection.md).
