...
### ComBat on previously unseen batches (NM 1.4)

NM 1.4 can now **apply ComBat to unseen batches at inference time**:
- Fit ComBat parameters on CV1 training data (optionally with retainment covariates).
- When a **new batch** appears in CV1/CV2 test folds or external data, NM applies **empirical Bayes mapping** using the learned priors to harmonize the new batch **without refitting** on test data.

Menu options:
- **Enable external‑batch adaptation** [yes/no]
- **Retainment covariates** (e.g., label) as in standard ComBat
- **Fallback policy** if batch id is missing

This supersedes previous recommendations that lacked an external‑validation mode; use with caution and always report train‑vs‑test batch distributions.
...
**4 | Geometric**: Component counts follow a geometric sequence between `[min, max]` (e.g., 8, 12, 18, 27, …).

**5 | Power**: Component counts follow a power‑law spacing across `[min, max]` (denser at small or large scales depending on exponent).

:::{note}
Use **geometric** and **power** options when the “right” scale is unknown and you want more coverage of early/late component ranges with fewer grid points. Applicable to PCA/NMF/RPCA; for PLS set latent‑dim grid analogously.
:::
...
(2004)](https://icml.cc/Conferences/2004/proceedings/papers/100.pdf)

- Utility Function: The Utility Function is the specific performance metric that NeuroMiner uses to evaluate and compare different models during the optimization process. In simple terms, it's the scorecard that determines which model is the "winner." When you use an optimization method like "Brute-force optimization," NeuroMiner trains many different model variations. The utility function provides the single, quantitative score (e.g., 91% accuracy) for each model, allowing the system to rank them and select the best one. Choosing the right utility function is crucial as it defines what "good performance" means for your specific problem: For a balanced classification problem (equal numbers in each group), accuracy might be a good choice. For an imbalanced classification problem, Balanced Accuracy or the Area Under the Curve (AUC) are often better choices as they are not skewed by the larger group. For a regression problem, you might use Pearson's correlation (to maximize the association between predicted and true values) or Root Mean Squared Error (to minimize the average prediction error).
...
2007](https://lig-membres.imag.fr/bisson/cours/M2INFO-AIW-ML/papers/Sun07.pdf)).
...
- F-Score Computation Method: This parameter lets you choose the statistical method for calculating the F-score.
    - mean(std): This is the classic F-score calculation, based on the mean and standard deviation of the feature's values within each group. It is powerful and works best when your data is normally distributed and does not contain significant outliers.
    - median(IQR): This is a "robust" alternative that uses the median and the Interquartile Range (IQR) instead. Because the median is less sensitive to extreme values than the mean, this method is recommended if you suspect your data contains outliers or is skewed.

- Weighting for Unbalanced Classes: This option adjusts the F-score calculation to account for groups of unequal sizes (e.g., 100 controls and 30 patients), a common scenario in clinical research.
    -  No weighting: Performs the standard calculation. This is suitable if your groups are of a similar size.
    - Automatic weighting: This option should be used for unbalanced classes. It modifies the F-score calculation to give more influence to the smaller group. This prevents the variance estimate from being dominated by the larger group, resulting in a more reliable and unbiased ranking of features.

- Stability-Enhanced F-Score (Bootstrapping): Feature ranks can be unstable, especially with smaller datasets. A feature might get a high rank by chance. This option uses bootstrapping to create a more stable and reliable estimate of feature importance. Here, you can define the number of bootstrap iterations (e.g., 100 or 1000). For each iteration, a random subsample of your data is drawn (with replacement), and F-scores are calculated. The final importance score for each feature is the average score across all iterations. This process reduces the likelihood that a feature is ranked highly due to random chance. Setting this value to 0 disables bootstrapping.
...
12. **ANOVA**:
Analysis of Variance (ANOVA) is a classic statistical method used to rank features based on how significantly their mean values differ across two or more groups. It is a powerful univariate filter, meaning it evaluates each feature independently. How it Works: For each feature, an ANOVA test is performed to compare the group means. The result of this test is an F-statistic. An F-statistic is essentially a ratio: the variance between the groups divided by the variance within the groups. A high F-statistic indicates that the difference between the group means is large relative to their internal variability, suggesting the feature is highly effective at discriminating between the groups. Features are then ranked in descending order of their F-statistic. When to Use It: ANOVA is a very fast and effective method for identifying features with strong, straightforward group differences. It is an excellent baseline or primary method for many classification problems. Parameters and Configuration: The main configuration option for ANOVA in NeuroMiner is the ability to provide a full design matrix instead of just a simple group label vector. This allows you to perform a more complex ANCOVA (Analysis of Covariance). By including nuisance variables (e.g., age, sex, scanner site) as columns in the design matrix, you can rank features based on their relationship with your groups of interest after controlling for the effects of those nuisance covariates.

13. **PLS**:
Partial Least Squares (PLS) is a powerful multivariate method that ranks features by identifying patterns across all features simultaneously that are maximally related to the outcome. Unlike univariate methods like ANOVA, which assess each feature in isolation, PLS excels at finding predictive signals in datasets with many, highly-correlated features (a common scenario in neuroimaging). How it Works: PLS reduces the data to a smaller set of "latent variables" or components. Each component is a weighted combination of the original features. The key is that these components are constructed to capture the maximum possible covariance between the feature data and the outcome variable (e.g., group labels or a continuous score). The importance of each original feature is then determined by its weight (or loading) on these predictive components. Features with a larger absolute weight contribute more to the pattern that predicts the outcome and are therefore ranked higher. When to Use It: PLS is ideal for high-dimensional datasets where the predictive information is likely distributed across many correlated features. It can uncover subtle, multivariate patterns that univariate methods would miss. Parameters and Configuration: The primary parameter to configure for PLS is the number of components to extract. This is a crucial hyperparameter that determines the complexity of the solution. In NeuroMiner, you can typically specify a range of values for the number of components (e.g., [1:10]), and the optimal number will be determined during model optimization.

14. **External ranking**:
The External Ranking option allows you to rank or weight your features using a pre-existing template or set of values, rather than calculating ranks from the training data itself. This is a powerful feature for incorporating prior knowledge into your model. For example, you could use a statistical map (e.g., a T-map from an SPM analysis), a gene-expression atlas, or results from a previous study to guide the feature weighting process. When you select this option, NeuroMiner provides three ways to load the external weights, which are detailed below.

- Use Weighting Map from Data Input: This option is available if you loaded a default weighting map when you first set up your data in NeuroMiner. The function will use that pre-loaded map to rank the features. Use Case: This is the most common method when you have a standard template or map (e.g., an anatomical atlas or a results map from a landmark study) that you intend to use for the entire project.

- Import External Weighting Map from File: This option opens a file selector, allowing you to choose a compatible map file (e.g., a NIFTI .nii or .img file for neuroimaging data) from your computer on-the-fly. NeuroMiner will load this file and use its values to rank your features. Use Case: This is ideal for one-off analyses or when you want to use a map that wasn't part of the original project setup, such as a statistical map you just generated from a separate group analysis.

- Read-in Weighting Vector from MATLAB Workspace: This option prompts you to enter the name of a variable that currently exists in your MATLAB workspace. This variable must be a vector with a length that exactly matches the number of features in your dataset. Use Case: This is the most flexible method, perfect for advanced or scripted analyses. You can use it to apply weights that you have calculated using a custom script, to use non-imaging data (like a list of gene-expression values), or to dynamically test different weighting schemes without having to save each one to a file.

:::{Important}
For any external ranking method, it is crucial to ensure that the dimensions and orientation of your external map or vector perfectly match those of your feature data. A mismatch in the number of elements or their spatial order will lead to incorrect feature weighting and invalid results.
:::
...
After a ranking algorithm assigns an importance score to every feature, thresholding is the critical step where you decide which features to keep for your model and which to discard. This process helps to simplify the model, reduce noise, and improve generalization. NeuroMiner supports several strategies for this.

- Hard Thresholding: This is the most intuitive and common method. You define a fixed cutoff value (the threshold). Any feature with an importance score above this threshold is kept, and any feature with a score below it is removed.

- Soft Thresholding: Soft thresholding is a more nuanced approach often used in regularization methods like the LASSO. It not only selects features but also shrinks their weights. Like hard thresholding, it sets any feature with an importance score below a certain threshold to zero. However, for the features that are kept, it reduces their score by the threshold amount, effectively pulling them closer to zero.

- Rank-Based Thresholding (Top-K or Percentile): This is a very popular variation of hard thresholding where the cutoff is relative rather than absolute. Instead of setting a specific score value as the threshold, you specify the quantity of features you want to keep. 
    - Top-K: You select a fixed number of the highest-ranking features (e.g., the "top 1000" features).
    - Percentile: You select a percentage of the highest-ranking features (e.g., the "top 5%" of features).
This approach is extremely useful because it guarantees that your final model will have a specific, predictable number of features, which helps in standardizing analyses and comparing results across different models or datasets.
...
Many machine learning algorithms perform best when the input features have a symmetric, bell-shaped (i.e., normal) distribution. Skewness is a measure of the asymmetry of a distribution. This preprocessing step aims to identify and correct highly skewed features, making them more symmetric and improving model performance and stability. How it Works: The process is applied to the training data within each cross-validation fold. First, NeuroMiner calculates the skewness of each feature. If a feature's absolute skewness value exceeds a user-defined threshold, a mathematical transformation is applied to that feature to reduce its skew. The parameters learned from the training data (e.g., the optimal lambda value) are then applied to the corresponding test data.

**Parameters and Options:**

1. Transformation Method

This is the core choice for how to correct the skew.

- Log Transform: A simple and fast method that applies the natural logarithm (log(x)) to the data. It is very effective for reducing right-skewness but has a key limitation: it can only be applied to strictly positive data.

- Box-Cox: A more flexible and powerful transformation controlled by a parameter called lambda (λ). It can correct for a wider variety of distributional shapes than a simple log transform. However, like the log transform, it also requires all data to be strictly positive.

- Yeo-Johnson: This is an extension of the Box-Cox transformation and is often the most versatile and recommended option. Its main advantage is that it works on data with both positive and negative values, making it applicable to a broader range of datasets without requiring manual data shifting.

2. Skewness Threshold

This numerical value determines which features will be corrected. The transformation is applied to any feature whose absolute skewness is greater than this threshold.

A common value is 2, but this can be adjusted depending on your data.

- Hyperparameter Optimization: You can enter a range of values here (e.g., [1 1.5 2]). NeuroMiner will then treat the threshold as a hyperparameter, automatically finding the value that results in the best model performance during the grid search.

3. Lambda (λ) Parameter (for Box-Cox & Yeo-Johnson)

The lambda parameter fine-tunes the shape of the Box-Cox and Yeo-Johnson transformations. For example, a λ of 0 corresponds to a log transform, while a λ of 0.5 corresponds to a square root transform.

- Auto: This is the default and recommended setting. NeuroMiner will automatically determine the optimal λ for each feature using a statistical method (Maximum Likelihood Estimation).

- Manual: This option is for advanced users who wish to specify the λ value(s) themselves. You can enter a single value or a range of values (e.g., [-1 -0.5 0 0.5 1]). If a range is provided, lambda will also be included as a hyperparameter in the model optimization process.
...
Choose the neurotransmitter maps you would like to use in your analysis. This step will only become available if a neurotransmitter maps directory has been selected.
...
:::{Important}
If you change the group processing mode, you'll need to repeat the setup of any feature selection steps.
:::
