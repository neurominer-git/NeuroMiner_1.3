...
## Back‑projection & permutation inference (NEW, NM 1.4)

- **Aggregated mode**: feature‑wise permutation test on the composite map \(w_{\mathrm{agg}}\).  
- **Component‑wise mode**: hierarchical selective‑inference —
  1) select significant components in **model space** (FDR across components),
  2) test features **within each selected component** with permutation‑based p‑values and FDR **per component**.  
  To mirror selection under the null, NM applies rank‑matched top‑K or threshold‑matched selection on each permutation before computing feature statistics.

> Tip: component‑wise tests avoid cross‑component cancellation and are often more sensitive than aggregated tests. See [Visualization Pathways](4.6.11_visualization_pathways_backprojection.md).
 ⁠
