...
12. NEURAL - [Python's sklearn.neural_network.MLPClassifier algorithm](https://scikit-learn.org/stable/modules/generated/sklearn.neural_network.MLPClassifier.html)
13. DEEP NN - [Tensorflow Sequential Models in Python](https://www.tensorflow.org/guide/keras/sequential_model)
:::
...
### TensorFlow models (NEW)

NM 1.4 adds **TensorFlow** models for deep learning (classification & regression):
- Model architecture and configuration: number of hidden layers and nodes, activation function, dropout, batch size, epochs, optimizer (Adam/SGD), learning rate schedules, early stopping.
- Uses MATLAB–Python bridge — ensure your Python env includes `tensorflow`.
- NeuroMiner Expert mode allows you to design your model architecture and parameters using a Python file Tensorflow definition. See *Learning algorithm parameters* for more information.

**Enabling GPU (optional)**
If your system has a supported GPU and CUDA/cuDNN, `tensorflow` will use it automatically once installed in the Python env that MATLAB is linked to (see *Python integration in MATLAB*). Training and inference happen inside CV1/CV2 without data leakage.

> Select **TensorFlow** in *Select learning algorithm*; then configure *Learning algorithm parameters* as usual.
