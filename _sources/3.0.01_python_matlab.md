...
## TensorFlow setup (NM 1.4)

To use TensorFlow models:
1. Install into the same Python environment that MATLAB uses:
```bash
   pip install tensorflow
```

2.	(GPU optional) Install a TF version compatible with your CUDA/cuDNN stack. Verify with:
...
3.	Confirm MATLAB is pointing to the correct Python via:

```
pe = pyenv;  % check Executable and Version

```


If imports fail, re‑link the environment:

```
pyenv("C:\Path\to\python.exe")  % or /path/to/python
