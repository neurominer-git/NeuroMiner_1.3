...
The initial "Subspace Selection Strategy" acts as a filter. You are selecting a pool of candidate models based on their individual, intrinsic properties (i.e., their standalone performance score) before building the final ensemble.
...
Specify what filter will be used to obtain the feature sets. 


1. Mutual information-based (FEAST toolbox): 
Selects features with the highest shared information with the target variable.
Implementation of the FEAST toolbox for feature selection
([site](http://www.cs.man.ac.uk/~gbrown/fstoolbox/)).

2. Maximum Relevance Minimum Redundancy (Peng et al): Pic
ks features that are both relevant to the target and minimally redundant with each other.

3. **Pearson / Spearman correlation**:
Ranks features based on their linear (Pearson) or monotonic (Spearman) correlation with the output.

4. Simba (margin-based method):
Scores features by how well they maximize class margins in a feature space.
...
5. G-flip (margin-based method):
Evaluates features by flipping class margins to find discriminative feature subsets.
See [Gilad-Bachrach et al.
(2004)](http://www.cs.huji.ac.il/labs/learning/Papers/giladbachrachnavottishby04b.pdf)
and
[site](http://www.cs.huji.ac.il/labs/learning/code/feature_selection.bak/)

6. IMRelief (margin-based method):
Uses instance-level weighting to assess feature importance based on local class margins.
...
Adds features one by one to find the optimal number for best model performance.

8. Random subspace sampling: 
Randomly selects feature subsets to promote ensemble diversity and robustness.

9. Relief for classification (margibn-based method):
Assigns feature weights by how well they separate near-hit and near-miss instances.
This is a version of the built-in MATLAB Relief algorithm
[(link](https://de.mathworks.com/help/stats/relieff.html)).

10. F-Score:
Ranks features by the ratio of between-class to within-class variance (signal-to-noise). 
...
11. Bhattacharya distance:
Measures feature discrimination based on the statistical distance between class distributions.
...
## New in NM 1.4 — Ensemble diversity & regularized selection

**Refactored diversity metrics** now available during greedy **forward selection** and **backward elimination**:
- **Vote Entropy**
- **Q‑statistic diversity**
- **Kappa diversity**
- **Bias–Variance decomposition**
- **Regression variance (for regression models)**

You can **combine performance + diversity** into a joint objective and **regularize** it to avoid overfitting:
- Joint score = `Perf – λ · Complexity + μ · Diversity`
  - Tune λ (complexity penalty) and μ (diversity weight) within CV1.
  - Works for **binary**, **regression**, and **multi‑class** ensembles.

**Where to configure**
- In *Filter‑based* step → **Optimize ensemble** → **Forward selection**/**Backward elimination** → **Optimization criterion**, pick **Performance + Diversity** and set **Regularization**.
- In *Wrapper‑based* step → **Wrapper type** (Greedy SA/Genetic/PSO/Pathfinder) → enable **Diversity‑aware objective** and **Min # base learners**.

**Notes**
- For *soft* ensembles use algorithm scores; for *hard* ensembles use label votes.
- Weighted ensembles can assign inverse‑error weights to selected base learners.
