...
switch meas{measind}
    ...
    case 'Model P value histogram'
         ...
         set(handles.pn3DView,'Visible','off'); set(handles.axes33,'Visible','on'); cla; hold on;
         ah=histogram(handles.axes33,y,'Normalization','probability','EdgeColor','none','FaceColor',cl_face);%,'FaceAlpha',0.5); 
         maxah= nm_nanmax(ah.Values); ylim([0 maxah]); 
         [f, xi] = ksdensity(y,'function','pdf'); 
         plot(handles.axes33,xi, scaledata(f,[],0, maxah),'Color',cl_edge, 'LineWidth',2);
    ...
    otherwise 
        ...
        switch v.params.visflag
            ...
            case {1,2}
                ...
                [pth,nam,ext] = fileparts(v.params.brainmask);
                switch ext
                    case '.nii'
                        st.fig = handles.pn3DView; 
                        st.NMaxes = [ handles.axes26 handles.axes27 handles.axes28];
                        handles.pn3DView.Visible='on'; handles.axes33.Visible='off'; handles.axes27.Visible='on'; handles.axes28.Visible='on';
