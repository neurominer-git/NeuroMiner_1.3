...
if ~isempty(uConstraint)
    ...
    minC = min(C,[],2);
    if any(minC==0)
        fprintf('\nNeuroMiner cannot generate your cross-validation matrix!!!\nThis are the case counts of your label-by-constrain value matrix:\n'); disp(C)
        error('Your constrain vector aligns with your labels! %g label values do not overlap with certain constrain vector values!', sum(minC==0));
    end
end  
