...
if ~isempty(analysis)
    ...
    for i=1:numel(analysis.params.TrainParam.PREPROC)
        inp.decompfl(i) = nk_DetIfDimRefInPREPROC(analysis.params.TrainParam.PREPROC{i}, i);
    end
    if (isscalar(inp.decompfl) && ~inp.decompfl) || sum(inp.decompfl)<numel(inp.decompfl)
