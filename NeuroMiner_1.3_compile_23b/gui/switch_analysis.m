...
if isfield(handles,'oocvview') && handles.oocvview && oocvind
    try
        if isfield(handles.NM.analysis{analind}.OOCV{handles.OOCVinfo.Analyses{analind}.OOCVvec(oocvind)},'MLI')
            mlidata = handles.NM.analysis{analind}.OOCV{handles.OOCVinfo.Analyses{analind}.OOCVvec(oocvind)}.MLI;
            mlifl = 'on';
...
% Check whether analysis involved probabilistic algorithm
[handles.params.algostr, handles.params.probflag] = determine_probflag(handles.NM.analysis{handles.curranal}.params.TrainParam.SVM);