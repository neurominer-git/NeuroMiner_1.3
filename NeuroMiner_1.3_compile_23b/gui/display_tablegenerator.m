            ...
            elseif isnumeric(SubgroupSelection) 
                ...
                if ~isempty(SubgroupSelectionNames ) || ~strcmp(SubgroupSelectionNames ,'')
                    ...
                    elseif any(strfind(SubgroupSelectionNames,'"'))
                        SubgroupSelectionNames = regexprep(SubgroupSelectionNames,'"','');
                ...
                if isfield(handles.NM.analysis{i},'OOCV') && ...
                    ...
                    switch handles.modeflag
                        case 'classification'
                            ...
                            %% CV: 25.10.2023 this condition is completely arbitrary 
                            if multiflag
                    ...
                    for p=1:nuSG
                        ...
                        for l=1:numel(MetricSelection) 
                            try
                                ...
                                fprintf('\nNot found for subgroup %s: %s',SubgroupSelectionDesc{p},MetricSelection{l});
                            end
                        end
                        cnt=cnt+1;
                    end
                    cnt=cnt-1;
                     ...
                     for l=1:numel(MetricSelection)
                         if handles.PerfTab.oocvselect.Value>1 
                             switch handles.modeflag
                                 case 'classification'
                                     data_table{cnt,l+1} = sfld{k}.(MetricSelection{l});
                                 case 'regression'
