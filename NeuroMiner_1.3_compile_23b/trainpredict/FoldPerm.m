...
if ~exist('OUT', 'var') || isempty(OUT)
    ...
    OUT.tr      = cell( IN.nperms, IN.nfolds, mnclass ); % Training performance
    OUT.mdl     = cell( IN.nperms, IN.nfolds, IN.nclass ); % Model structures
    OUT.Trtargs = cell( IN.nperms, IN.nfolds, mnclass ); % Predicted target labels for CV1 training data
    OUT.Trdecs  = cell( IN.nperms, IN.nfolds, mnclass ); % Decision values / probabilities for CV1 training data
    OUT.kxVec   = cell( IN.nperms, IN.nfolds, IN.nclass ); % Subspace Stepping
    OUT.ts      = cell( IN.nperms, IN.nfolds, mnclass ); % Test performance
    OUT.rf      = cell( IN.nperms, IN.nfolds, IN.nclass ); % Recursive feature elimination structures (unused)
    OUT.w2      = cell( IN.nperms, IN.nfolds, mnclass ); % |w| for linear SVM kernels
    OUT.Md      = cell( IN.nperms, IN.nfolds, mnclass ); % distance to hyperplane for linear SVM kernels
    OUT.Mm      = cell( IN.nperms, IN.nfolds, mnclass ); % normalized margin for linear SVM kernels
    OUT.CVtargs = cell( IN.nperms, IN.nfolds, mnclass ); % Predicted target labels for CV1 test data
    OUT.CVdecs  = cell( IN.nperms, IN.nfolds, mnclass ); % Decision values / probabilities for CV1 test data
                ...
                for kT=1:kFea(curclass)
                    ...
                    if VERBOSE 
                        switch MODEFL
                            case 'classification'
                               if RAND.Decompose ~= 9
                                    fprintf('\n%s => CV1 [%g, %g, %s, Subspace %g/%g (%g feats)]:', strout, ...
                    ...
                    % Store number of features in OUT if no wrapper-based
                    % mask available or detected
                    if isempty(RF) || ~RF.found
                        for v=1:IN.nvar 
                             OUT.featnum{i,j,curclass,v}(indkX) = sum(Fk{curclass, v}(:,indkX)~=0);
...
switch RAND.Decompose
    case 9
        ...
        tr_label = tTrL{curclass}; cv_label = tCVL{curclass};
        ...
        for m_curclass=1:nclass
            ...
            Ltr_curclass = zeros(size(tr_label)); Ltr_curclass(tr_label==m_curclass)=1; Ltr_curclass(Ltr_curclass==0) = -1;
            tr = BAC(Ltr_curclass, tr_decs(:,indkX, m_curclass));  
            if isnan(tr), warning('Binary comparison %g: Non-finite performance measures found in CV1 training data', m_curclass); end
            if VERBOSE, fprintf('\tTr(class %g) = %1.2f', m_curclass, tr); end
            ...
            if ~fFull || RetrainImmediate || cvts_fl(curclass)
                Lcv_curclass = zeros(size(cv_label)); Lcv_curclass(cv_label==m_curclass)=1; Lcv_curclass(Lcv_curclass==0) = -1;
                ts = BAC(Lcv_curclass, cv_decs(:, indkX, m_curclass));  
                if isnan(ts), warning('Binary comparison %g: Non-finite performance measures found in CV1 test data', m_curclass); end
                if VERBOSE, fprintf('\tCV(class %g) = %1.2f', m_curclass, ts); end
