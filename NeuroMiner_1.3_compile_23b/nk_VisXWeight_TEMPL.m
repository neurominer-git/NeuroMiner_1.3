function [ mW, mP, mR, mSR, mC, W, mPA, mWF ] = nk_VisXWeight(inp, MD, Y, L, varind, P, F, VI, decompfl, memprob, procfl, Fadd)
% =========================================================================
% [mW, mP, mR, mSR, mC, W, mPA, mWF] = nk_VisXWeight(inp, MD, Y, L, varind, ...
...
mP = []; mR = []; mSR = []; mC=[]; mWF=[]; templfl = false;
if procfl
    mP = cell(1,mM); mR = cell(1,mM); mSR = cell(1,mM); mC = cell(1,nM);
    if any(decompfl) && ~isempty(TEMPL)
        mWF = cell(1,mM); templfl = true;
    end
end
                ...
                case {'reducedim','remvarcomp'}
                    ...
                    if isfield(naPX,'recon') && naPX.recon==1
                        ...
                        if isfield(naPX.mpp,'vec')
                           redvec = naPX.mpp.vec;
                           if templfl, templvec = TEMPL.TemplParams{a}.mpp.vec; end
                        elseif isfield(naPX.mpp,'factors')
                           redvec = naPX.mpp.factors{1};
                           if templfl, templvec = TEMPL.TemplParams{a}.mpp.factors{1}; end
                        elseif isfield(naPX.mpp,'u')
                           redvec = naPX.mpp.u;
                           if templfl, templvec = TEMPL.TemplParams{a}.mpp.u; end
                        elseif isfield(naPX.mpp,'M')
                           redvec = naPX.mpp.M;
                           if templfl, templvec = TEMPL.TemplParams{a}.mpp.M; end
                        elseif isfield(naPX.mpp,'W')
                            redvec = naPX.mpp.W;
                            if templfl, templvec = TEMPL.TemplParams{a}.mpp.W; end
                        ...
                        if templfl
                            [~, IN] = nk_SortComponents(templvec,mpp.vec,[]);
                            if ~isfield(TEMPL,'templmatrixsize'), TEMPL.templmatrixsize = IN.templmatrixsize; end
                            if numel(nmW)>IN.templmatrixsize(2)
                                nmWF = nmW(IN.mapping);
                            else
                                nmWF = nan( 1, IN.templmatrixsize(2) );
                                nmWF(IN.mapping) = nmW;
                            end
                        end
...
%                             if templfl
%                                 tnmWF = zeros(size(naPX.indNonRem')); 
%                                 tnmWF(naPX.indNonRem) = nmWF; nmWF = tnmWF;
%                             end
                            clear tmW tmP tmPA 
                        end
                        reducedimfl = true;
...
%                     if templfl && reducedimfl
%                          tnmWF = zeros(length(pIND)); 
%                          tnmWF(pIND) = nmWF; nmWF = tnmWF;
%                     end
        ...
        if nM == 1 && mM > 1
            for m=1:numel(varind)
                if procfl
                    ...
                    %if templfl, mWF{m} = nmWF(indX); end
                else
                    mW{m} = nmW;
                    mP{m} = nmP;
                    mR{m} = nmR;
