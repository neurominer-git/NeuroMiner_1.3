...
for curlabel=1:nl
    ...
    for curclass=1:nclass
        ...
        switch MODEFL
            ...
            case 'classification'
                if RAND.Decompose ~= 9
                    ...
                    fprintf('**** Selected binary classifiers #%g (%s) ****', ...
                        curclass, tCV.class{f,d}{curclass}.groupdesc)
                else
                    fprintf('\n\n');
            ...
            case 'regression'
                fprintf('\n\n');
                fprintf('**** Selected regression models ****')
        end
...
%                 end
        ...
        if nPerc > 1
           ...
           fprintf('Final transnode CV1 ensemble performance %g at %g-percentile using %g node(s).', ...
    ...
    if MULTI.flag
       ...
       if MULTI.BinBind
           ...
           [op, ~, ~, PerfCritMult] = nk_ReturnEvalOperator(SVM.GridParam);
           for PercStep = 1:nPerc
